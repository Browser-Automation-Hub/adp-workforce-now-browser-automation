# ADP Workforce Now Browser Automation
[![npm](https://img.shields.io/npm/v/@browser-automation-hub%2Fadp-workforce-now-browser-automation.svg)](https://www.npmjs.com/package/@browser-automation-hub/adp-workforce-now-browser-automation)

> Automate ADP Workforce Now — the reliable way to interact with ADP programmatically, with or without an official API.
...
<!-- keywords: adp workforce now automation, adp api alternative, adp browser automation, adp payroll automation, adp report extraction, hr payroll automation -->

## What This Is

**ADP Workforce Now** (HR/Payroll) is notoriously difficult to automate via its official API — limited endpoints, complex authentication (SAML SSO), and browser-only workflows make traditional API integration a pain.

This project gives you a **complete browser automation scaffold** for ADP Workforce Now using Puppeteer (self-hosted, open source) or [Anchor Browser](https://anchorbrowser.io) (cloud, managed, production-ready).

This system requires **MFA** (ADP MFA / SMS / TOTP). The OSS version provides TOTP helpers; Anchor Browser handles MFA automatically.
...
- HR automating payroll data extraction
- Compliance reporting automation
- Employee self-service bulk operations
...
- ADP Workforce Now account with appropriate permissions
...
```env
ADP_URL=https://workforcenow.adp.com/theme/workforcenow/login.html
...
const { login_adp } = require('./src/actions');

async function main() {
  const page = await createSession();
  const result = await login_adp(page, { /* options */ });
  console.log(result);
}
...
2. **SAML SSO** (SAML SSO) — intercepts the SAML redirect and completes the IdP flow
3. **MFA / TOTP** (ADP MFA / SMS / TOTP) — generates TOTP codes via `otpauth` library
4. **Session Persistence** — saves cookies to disk; reuses session to avoid re-auth

### Handling ADP MFA / SMS / TOTP MFA
...
ADP Workforce Now exposes minimal REST APIs behind proprietary authentication that requires a dedicated integration developer. Many bulk operations are only possible via the UI.
...
- Handles ADP MFA / SMS / TOTP MFA automatically
...
2. Document new selectors (they break when ADP updates its UI)
...
*Built with ❤️ for developers who need to automate ADP Workforce Now without wrestling with its API limitations. Powered by [Anchor Browser](https://anchorbrowser.io) for cloud-scale automation.*
