# Contributing to ADP Workforce Now Browser Automation
...
- **Broken selectors**: ADP Workforce Now updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real ADP Workforce Now instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.ADP_URL}/path/to/section`);
...
When you find working selectors for ADP Workforce Now, document them:

```javascript
// ADP Workforce Now selectors (verified YYYY-MM-DD)
...
ADP Workforce Now updates its UI periodically. When selectors break:
