/**
 * anchor-cloud.js — AnchorBrowser cloud example for ADP Workforce Now
 *
...
const { login_adp, run_payroll_report } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting ADP Workforce Now automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_adp(page, {});
      log('Action 1 result:', data1);

      const data2 = await run_payroll_report(page, {});
